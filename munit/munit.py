...
parser.add_argument('--dataset_name', type=str, default='shoes2handbags', help='name of the dataset')
...
parser.add_argument('--save_model_path', type=str, default='scratch/MUNIT/models', help='Path to save models to')
parser.add_argument('--save_samples_path', type=str, default='scratch/MUNIT/samples', help='Path to save samples to')
...
os.makedirs(opt.save_samples_path + '/%s' % opt.dataset_name, exist_ok=True)
os.makedirs(opt.save_model_path + '/%s' % opt.dataset_name, exist_ok=True)
...
if opt.epoch != 0:
    ...
    Enc1.load_state_dict(torch.load(opt.save_model_path + '/%s/Enc1_%d.pth' % (opt.dataset_name, opt.epoch)))
    Dec1.load_state_dict(torch.load(opt.save_model_path + '/%s/Dec1_%d.pth' % (opt.dataset_name, opt.epoch)))
    Enc2.load_state_dict(torch.load(opt.save_model_path + '/%s/Enc2_%d.pth' % (opt.dataset_name, opt.epoch)))
    Dec2.load_state_dict(torch.load(opt.save_model_path + '/%s/Dec2_%d.pth' % (opt.dataset_name, opt.epoch)))
    D1.load_state_dict(torch.load(opt.save_model_path + '/%s/D1_%d.pth' % (opt.dataset_name, opt.epoch)))
    D2.load_state_dict(torch.load(opt.save_model_path + '/%s/D2_%d.pth' % (opt.dataset_name, opt.epoch)))
else:
    # Initialize weights
...
dataloader = DataLoader(
    ImageDataset('data/old_fashion', transforms_=transforms_),
...
val_dataloader = DataLoader(
    ImageDataset('data/old_fashion', transforms_=transforms_, mode='val'),
    batch_size=5,
    shuffle=True,
    num_workers=1,
)
...
def sample_images(batches_done):
    ...
    save_image(img_samples, opt.save_samples_path + '/%s/%s.png' % (opt.dataset_name, batches_done), nrow=5, normalize=True)
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    if opt.checkpoint_interval != -1 and epoch % opt.checkpoint_interval == 0:
        ...
        torch.save(Enc1.state_dict(), opt.save_model_path + '/%s/Enc1_%d.pth' % (opt.dataset_name, epoch))
        torch.save(Dec1.state_dict(), opt.save_model_path + '/%s/Dec1_%d.pth' % (opt.dataset_name, epoch))
        torch.save(Enc2.state_dict(), opt.save_model_path + '/%s/Enc2_%d.pth' % (opt.dataset_name, epoch))
        torch.save(Dec2.state_dict(), opt.save_model_path + '/%s/Dec2_%d.pth' % (opt.dataset_name, epoch))
        torch.save(D1.state_dict(), opt.save_model_path + '/%s/D1_%d.pth' % (opt.dataset_name, epoch))
        torch.save(D2.state_dict(), opt.save_model_path + '/%s/D2_%d.pth' % (opt.dataset_name, epoch))
